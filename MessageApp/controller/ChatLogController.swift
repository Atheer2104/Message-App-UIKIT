...
//  MessageApp
//
//  Created by atheer on 2018-01-04.
    ...
    private func handleVideoSelectedForUrl(url: URL) {
        ...
        let uploadTask = Storage.storage().reference().child("message_movies").child(filename).putFile(from: url, metadata: nil, completion: { (metadata, error) in
            ...
            if let videoUrl = metadata?.downloadURL()?.absoluteString {
                ...
                if let thumbanilImage = self.thumbnailImageForfileUrl(fileUrl: url) {
                    
                    self.uploadToFirebaseStorageUsingImage(image: thumbanilImage, completion: { (imageUrl) in
                        
                        let properties = ["imageUrl": imageUrl, "imageWidth": thumbanilImage.size.width, "imageHeight": thumbanilImage.size.height, "videoUrl": videoUrl] as [String : AnyObject]
...
//        NotificationCenter.default.addObserver(self, selector: #selector(handleKeyboardWillHide), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let text = message.text {
            ...
            // gets in here if it is an image
    ...
    private func sendMessageWithProperties(properties: [String: AnyObject]) {
        ...
        // append properties to value, but how???
        // key $0, value $1
    ...
    //zooming func in here don't want a viewto run that kind of code
...
//                do nothing
    ...
    @objc func handelZoomOut(tapGestuter: UITapGestureRecognizer) {
        if let zoomingOutView = tapGestuter.view {
